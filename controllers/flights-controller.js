...
exports.fetchFlightsList = async (req, res) => {
  try {
    ...
    const [nonStopResponse, oneStopResponse] = await Promise.all([
      fetchFlights(flightSearchParams, true),
      fetchFlights(flightSearchParams, false),
    ]);

    if (!nonStopResponse?.data && !oneStopResponse?.data) {
      return res.status(404).json({ message: "No flights available" });
    }

    let flights = [
      ...(nonStopResponse.data || []),
      ...(oneStopResponse.data || []),
    ];
...
async function fetchFlights(params, nonStop) {
  return amadeus.shopping.flightOffersSearch.get({ ...params, nonStop });
}
